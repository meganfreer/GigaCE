...
class separationview(tk.Frame):
   ...
   Lists all items in queue as a table.
       INFORMATION
       NAME | BUFFER | INJ VOLUME | INSTRUMENT | PLATES
...
"""
   ...
   def __init__(self, parent, controller, session, user, engine, *args, **kwargs):
       ...
       button2 = Button(self.peakframe, text="Export Electropherograms", command=self.export_separations, width=18)
       ...
       # Add in Giga-CE Help button here
       Helpbutton = Button(self.separationframe, text="Giga-CE Help", command=self.open_help)
       Helpbutton.grid(row=1, column=2, sticky="NSEW")
 
       self.peak_edit(0, initial=True)
       # self.tree.grid()
       
       # save overall poly value, autopopulation boolean, degree, and skip/"First Data to Skip"
       self.poly_value_for_all_files = None
       self.poly_autopopulate_for_all_files = None
       self.degree_for_all_files = None
       self.skip_for_all_files = None
   ...
   def open_help(self):
       filename = "GigaCE Instructions.docx"
       subprocess.run(['open', filename], check=True)
 
   def newpeak(self):
       """Called to create a new peak
       Creates a new peak instance in db
       Can be used to create multiple peaks at once
       reloads peak tables"""

       # When separations List tree is clicked, highlighed rows will be used to
       # obtain all separation id's for the higlighed rows
       sesh = self.session
       selection = self.tree.selection()  # get selected rows
       queue = []
       # print("Selection is ", selection)
       first = True
       # "item" is a string (separation/run ID)
       for item in selection:
           self.tree.item(item)
           if first:
               try:
                   peak = db.Peak(name="New", run_id=item)
                   sesh.add(peak)
                   sesh.commit()
                   self.fillpeakstable(self.peaksqueue) # Pass to load peaks
   ...
   def separation_selection(self, event):
       ...
       # print("Selection is ", selection)
       first = True
       # "item" is a string (separation ID)
       for item in selection:
           self.tree.item(item)
           if first:
               try:
   ...
   def separation_edit(self, sep_id):
       ...
       '''
       groupingbutton = Button(editframe, text = "Add Groupings",
                         command=self.add_groupings)
       groupingbutton.grid(row=1,column=5)
       '''
       ...
       if instance.kwds != None:
           ...
           try:              
               # reset all  boxes to have nothing in them
               self.polyspin.delete(0, "end") # degrees
               self.skipspin.delete(0, "end") # skip
               self.poly_value.set(False) # polynomial boolean (if manually entered)
               self.poly_autopopulate_value.set(False) # autopopulate boolean

               if kwds["poly_value"] == 1: # if not autopopulating (values directly input)
                   self.polyspin.insert(0, eval(kwds["degree"]))
                   self.skipspin.insert(0, eval(kwds["skip"]))

                   self.poly_value.set(kwds["poly_value"])
                   self.poly_autopopulate_value.set(kwds["poly_autopopulate_value"])
               
               # if autopopulation has occurred (but some may be manually changed after)
               elif self.poly_autopopulate_for_all_files == 1:
                   # if autopopulated to have a baseline (rather than autopopulated to take away baseline) 
                   if self.poly_value_for_all_files == 1:
                       # display previously entered in values for degrees, skip, etc
                       self.polyspin.insert(0, self.degree_for_all_files)
                       self.skipspin.insert(0, self.skip_for_all_files)

                       self.poly_value.set(self.poly_value_for_all_files)
                       self.poly_autopopulate_value.set(self.poly_autopopulate_for_all_files)
               
               else:
                   # if neither already filled nor autopopulating
                   print("No baseline (neither manual nor autopopulate) for this separation")
   ...
   def peak_edit(self, peak_id, initial=False):
       ...
       try:
           if self.peak_edit_first == True:
               ...
               saveeditsbutton = Button(editframe, text="Save Peak Edits",
               ...
               try:
                   ...
                   self.poly_autopopulate_value = IntVar(editframe)
                   self.poly_autopopulate_value.set(0)
                   poly_autopopulate_button = Checkbutton(editframe, text="Autopopulate Poly Baseline to Session",
                                             variable=self.poly_autopopulate_value, command=self.poly_autopopulate)
                   poly_autopopulate_button.grid(row=0, column=7, columnspan=4)
   ...
   def poly_autopopulate(self):
       return
 
   def delete_peak(self):
       """ When peak List tree is clicked, highlighed rows will be used to obtain all
       peak id's for the higlighed rows and set the width for each peak"""
       sesh = self.session
       selection = self.treepeak.selection()  # get selected peaks/rows
       queue = []
       first = True
       # "item" is a string (peak ID)
       for item in selection:
           self.treepeak.item(item)
           if first:
               try:
                   instance = sesh.query(db.Peak).filter(db.Peak.id == item)
                   instance = instance.first()
                   self.peakinstance = instance
                   ...
                   if item == self.selection:
                       ...
                       self.selection = item  # Allow first selection item to be edited
           queue.append(item)  # append peak ID (from 2nd column) to queue  
   ...
   def save_edits(self):
       ...
       kwds["poly_autopopulate_value"] = self.poly_autopopulate_value.get()
       ...
       # save overall poly value and degrees/skip values from user input
       self.poly_value_for_all_files = self.poly_value.get()
       self.poly_autopopulate_for_all_files = self.poly_autopopulate_value.get()
       self.degree_for_all_files = int(self.polyspin.get())
       self.skip_for_all_files = int(self.skipspin.get())
   ...
   def add_groupings(self):
       '''
       pop up window
       '''
       # Open dialog box to get groupings
       # could return something from this if you want to store it
       # (such as volume with other pop-up)
       GroupingRuns.groupingwindow(self)
       ...
       for instance in sesh.query(db.Separation).filter(db.Separation.id.in_(queue)):
           ...
           try:
               if instance.kwds != None:
                   ...
                   if kwds["poly_value"] == 1: # if not autopopulating (valeus directly input)
                       poly_value = 1
                   ...
                   # if autopopulation has occurred (but some may be manually changed after)
                       ...
                       if self.poly_value_for_all_files == 1:
                           poly_value = 1
                           # set degree and skip to autopopulated degree and skip values (used for finding baseline)
                           degree = self.degree_for_all_files
                           skip = self.skip_for_all_files                     
                   else:
                       poly_value = False
           ...
           if poly_value == 1:
               ...
               data[-instance.id] = [instance.name + 'BASELINE', Time, baseline]
           RFU = new_baseline.correctedrfu
 
           #data[instance.id] = [instance.shortname, Time, RFU]
           data[instance.id] = [instance.name, Time, RFU]
       self.rfu_data = data
   ...
   def save_peakedits(self):
       """ When peaks List tree is clicked, highlighed rows will be used to obtain all
       peak id's for the higlighed rows and save the peak information"""
       sesh = self.session
       selection = self.treepeak.selection()  # get selected rows
       queue = []
       first = True
       ...
       for item in selection:
           ...
           if first:
               try:
                   ...
                   coords = self.coords
                   inst = sesh.query(db.Peak).filter(db.Peak.id == item)
                   inst = inst.first()
   ...
   def plotclick(self, event):
       ...
       if self.left_flag:
           ...
           try:
               ...
               self.vline1 = None
           self.vline1 = self.plotsubplot.vlines(event.xdata, ymax * .01, ymax * 0.99, color='b')
           ...
           try:
               ...
               self.vline2 = None
           self.vline2 = self.plotsubplot.vlines(event.xdata, ymax * .01, 0.99 * ymax, color='b')
